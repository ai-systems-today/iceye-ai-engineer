...
- [x] Zip file representing the current repo state has been created
...
- [x] Repository has [tests](tests) sub-directory with one unit test for each task
...
- [x] [README.md](README.md) documents install, execute, and test flows for the current repository state
...
9. One unit test per task: complete
10. README completion: complete
11. Final compliance verification: complete
...
### Step 9: one unit test per task

- Verified [tests/test_part1.py](tests/test_part1.py) covers normalized unique-entity output and question-driven mention counting.
- Verified [tests/test_part2.py](tests/test_part2.py) covers two-blob detection and CSV header generation.
- Verified [iceye_ai_engineer/__init__.py](iceye_ai_engineer/__init__.py) and [tests/conftest.py](tests/conftest.py) make the local package importable during pytest collection.

### Step 10: README completion

- Verified [README.md](README.md) documents install, execute, and test commands.
- Verified [README.md](README.md) includes a step-by-step description of how the tests were created.

### Step 11: final compliance verification

- Verified `uv run pytest` passes.
- Verified `py -3.12 -m uv sync` still passes.
- Verified the current delivery archive is [iceye-ai-engineer-current-state.zip](iceye-ai-engineer-current-state.zip).